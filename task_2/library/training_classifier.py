...
from .evaluation_classifier import evaluate_classifier

logger = logging.getLogger(__name__)


def train_classifier(train_dataset, model, tokenizer, train_batch_size: int,
                     model_type: str, model_name_or_path: str, output_dir: str, predict_file: Path, log_file: Dict,
                     device: torch.device, max_steps: Optional[int], gradient_accumulation_steps: int,
                     num_train_epochs: int,
                     warmup_steps: int, logging_steps: int, save_steps: int, evaluate_during_training: bool,
                     max_seq_length: int, eval_batch_size: int,
    ...
    no_scaled_lr = ["cause_effect_classifier"]
            ...
            if (step + 1) % gradient_accumulation_steps == 0:
                ...
                if (logging_steps > 0 and global_step % logging_steps == 0) or \
                    ...
                    if evaluate_during_training:
                        metrics = evaluate_classifier(model=model,
                                                      tokenizer=tokenizer,
                                                      device=device,
